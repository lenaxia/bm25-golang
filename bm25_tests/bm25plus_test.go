...
func TestNewBM25Plus(t *testing.T) {
    ...
    _, err := bm25.NewBM25Plus(corpus, tokenizer, -1.0, 0.75, 1.0, 0.5, nil)
    if err == nil {
    ...
    _, err = bm25.NewBM25Plus(corpus, tokenizer, 1.2, 1.5, 1.0, 0.5, nil)
    if err == nil {
    ...
    _, err = bm25.NewBM25Plus(corpus, tokenizer, 1.2, 0.75, -1.0, 0.5, nil)
    if err == nil {
...
func TestBM25PlusGetScores(t *testing.T) {
    corpus := []string{"hello world", "this is a test"}
    tokenizer := func(s string) []string { return strings.Split(s, " ") }
    bm25, _ := bm25.NewBM25Plus(corpus, tokenizer, 1.2, 0.75, 1.0, 0.5, nil)
