...
func TestNewBM25L(t *testing.T) {
	...
	_, err := NewBM25L(corpus, tokenizer, k1, b, logger)
	if err != nil {
		t.Errorf("NewBM25L() failed: %v", err)
	}
}

func TestBM25L_GetScores(t *testing.T) {
	corpus := []string{"hello world", "goodbye world"}
	tokenizer := func(s string) []string {
		return strings.Split(s, " ")
	}
	...
	bm25, _ := NewBM25L(corpus, tokenizer, k1, b, logger)
	query := []string{"hello"}
...
func TestBM25L_GetBatchScores(t *testing.T) {
...
func TestBM25L_GetTopN(t *testing.T) {
	corpus := []string{"hello world", "goodbye world", "another doc"}
	tokenizer := func(s string) []string {
		return strings.Split(s, " ")
	}
