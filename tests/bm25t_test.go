...
func TestNewBM25T(t *testing.T) {
	...
	_, err := NewBM25T(corpus, tokenizer, k1, b, delta, logger)
	if err != nil {
		t.Errorf("NewBM25T() failed: %v", err)
	}
}

func TestBM25T_GetScores(t *testing.T) {
	corpus := []string{"hello world", "goodbye world"}
	tokenizer := func(s string) []string {
		return strings.Split(s, " ")
	}
	...
	bm25, _ := NewBM25T(corpus, tokenizer, k1, b, delta, logger)
	query := []string{"hello"}
...
func TestBM25T_GetBatchScores(t *testing.T) {
...
func TestBM25T_GetTopN(t *testing.T) {
	corpus := []string{"hello world", "goodbye world", "another doc"}
	tokenizer := func(s string) []string {
		return strings.Split(s, " ")
	}
